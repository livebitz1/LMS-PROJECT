...
export async function POST(req: NextRequest) {
  try {
    ...
    const { firstName, lastName, displayName, bio, degree, experienceYears, subjects, linkedin, skills, hourlyRate } = body || {};
    ...
    if (tp && typeof tp.upsert === 'function') {
      ...
      // Check existing
      const existing: TeacherProfileRow[] = await prisma.$queryRaw`SELECT id, "userId", "displayName", bio, degree, "experienceYears", "hourlyRate", subjects, skills, linkedin, "createdAt", "updatedAt" FROM "TeacherProfile" WHERE "userId" = ${userId} LIMIT 1`;
      if (existing && existing.length > 0) {
        await prisma.$executeRaw`
          ...
          WHERE "userId" = ${userId}
        `;
        const rows: TeacherProfileRow[] = await prisma.$queryRaw`SELECT id, "userId", "displayName", bio, degree, "experienceYears", "hourlyRate", subjects, skills, linkedin, "createdAt", "updatedAt" FROM "TeacherProfile" WHERE "userId" = ${userId} LIMIT 1`;
        profile = rows[0] ?? null;
      } else {
        const newId = randomUUID();
        await prisma.$executeRaw`
          INSERT INTO "TeacherProfile" (id, "userId", "displayName", bio, degree, "experienceYears", "hourlyRate", subjects, skills, linkedin, "createdAt", "updatedAt")
          VALUES (${newId}, ${userId}, ${profileData.displayName}, ${profileData.bio}, ${profileData.degree}, ${profileData.experienceYears}, ${profileData.hourlyRate}, ${subjectsJson}::jsonb, ${skillsJson}::jsonb, ${profileData.linkedin}, now(), now())
        `;
        const rows: TeacherProfileRow[] = await prisma.$queryRaw`SELECT id, "userId", "displayName", bio, degree, "experienceYears", "hourlyRate", subjects, skills, linkedin, "createdAt", "updatedAt" FROM "TeacherProfile" WHERE "userId" = ${userId} LIMIT 1`;
        profile = rows[0] ?? null;
      }
